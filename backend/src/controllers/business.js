...
// ======================================================
// SAVE / UPDATE BUSINESS DRAFT (Draft Flow - Optional Fields)
// ======================================================
exports.saveBusinessDraft = async (req, res) => {
  try {
    const userId = req.user.id;
...
// ======================================================
exports.updateDashboardBusinessDetails = async (req, res) => {
  try {
    const userId = req.user.id;
    const {
      address,
    ...
    // Partial updates - jo aayega bas wahi change hoga
    if (address !== undefined) business.address = address.trim();
    if (currentCity !== undefined) business.currentCity = currentCity.trim();
    if (currentState !== undefined) business.currentState = currentState.trim();
    if (pincode !== undefined) business.pincode = pincode.trim();

    // Alternate Mobile Numbers (Array update)
    if (Array.isArray(alternatePhoneNumbers)) {
      business.alternatePhoneNumbers = alternatePhoneNumbers
    ...
    console.error("UPDATE DASHBOARD DETAILS ERROR:", error);
    return res.status(500).json({
      success: false,
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        // Contact Info (Primary + Alternate Array)
        phoneNumber: business.phoneNumber || "",
        alternatePhoneNumbers: business.alternatePhoneNumbers || [],
        email: business.email || "",
