...
exports.createBid = async (req, res) => {
    try {
        ...
        // "Open" AUR "Pending" dono status par bid allow karein
...
exports.updateBid = async (req, res) => {
    try {
        ...
        // Sirf apni bid edit kar sakta hai
        if (bid.transporter.toString() !== req.user.id) {
            return res.status(403).json({
                success: false,
                message: "You can only edit your own bid."
            });
        }

        // Accepted / Rejected ke baad edit nahi
        ...
        return res.status(500).json({
            success: false,
            message: "We couldn't process your request at the moment. Please try again later."
        });
    }
};
...
exports.deleteBid = async (req, res) => {
    try {
        ...
        // Sirf apni bid delete kar sakta hai
        if (bid.transporter.toString() !== req.user.id) {
            return res.status(403).json({
                success: false,
                message: "You can only delete your own bid."
            });
        }

        // Accepted / Rejected ke baad delete nahi
        if (bid.status !== "Pending") {
            return res.status(400).json({
                success: false,
...
exports.getLeadBids = async (req, res) => {
    try {
        ...
        // Sirf lead creator/owner hi bids dekh sake
        ...
        const result = await Promise.all(
            bids.map(async (bid) => {
                return {
                    ...bid.toObject(),
                    transporter: {
                        ...(bid.transporter ? bid.transporter.toObject() : {}),
...
exports.acceptBid = async (req, res) => {
    try {
        ...
        if (booking.status !== "Open" && booking.status !== "Pending") {
            return res.status(400).json({
                success: false,
